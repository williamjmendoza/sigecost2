<?php
	...
	class ModeloInstanciaETBarraFormato
	{
		...
		public static function actualizarInstancia(EntidadInstanciaETBarraFormato $instancia)
		{
			$preMsg = 'Error al actualizar la instancia de elemento tecnológico barra de formato.';
			...
			try
			{
				...
				$query = '
						...
						WHERE
						{
							?iri rdf:type :'.SIGECOST_FRAGMENTO_BARRA_FORMATO.' .
							?iri :nombreAplicacionPrograma ?nombreAplicacion .
						 	?iri :versionAplicacionPrograma ?versionAplicacion .
				...
				*/
				...
				$GLOBALS['SigecostInfo']['general'][] = "Instancia de barra de dibujo modificada satisfactoriamente.";
		...
		{
			$preMsg = 'Error al buscar el contador de las instancias elemento tecnologico de barra de formato.';

			// Buscar la cantidad de instancias de elemento tecnologico barra de formato.
			$query = '
					PREFIX : <'.SIGECOST_IRI_ONTOLOGIA_NUMERAL.'>
					PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
		...
		{
			$preMsg = 'Error al eliminar la instancia de elemento tecnol&oacute;ico barra de formato.';
			...
			try
			{
				...
				if($result["result"]["t_count"] == 0) {
					// Excepción porque no se pudieron borrar los datos de la instancia, 
					throw new Exception($preMsg . ' Detalles: No se eliminó ningún registro.');
				}
