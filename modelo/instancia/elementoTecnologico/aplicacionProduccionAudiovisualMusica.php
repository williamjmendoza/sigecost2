<?php
	...
	class ModeloInstanciaETAplicacionProduccionAudiovisualMusica
	{
		...
		public static function actualizarInstancia(EntidadInstanciaETAplicacionProduccionAudiovisualMusica $instancia)
		{
			$preMsg = 'Error al actualizar la instancia de elemento tecnológico en aplicación producción audiovisual y música.';
			...
			try
			{
				...
				$query = '
						...
						WHERE
						{
							?iri rdf:type :'.SIGECOST_FRAGMENTO_APLICACION_PRODUCCION_AUDIOVISUAL_MUSICA.' .
							?iri :nombreAplicacionPrograma ?nombreAplicacion .
						 	?iri :versionAplicacionPrograma ?versionAplicacion .
		...
		{
			$preMsg = 'Error al eliminar la instancia de elemento tecnol&oacute;ico aplicaci&oacute;n producci&oacute;n audiovisual y m&uacute;sica.';
		...
		{
			$preMsg = 'Error al verificar si está siendo utilizada una instancia de aplicación producción audiovisual y musica.';
			...
			try
			{
				...
				$query = '
						...
						{
							?instanciaAplicacion rdf:type :'.SIGECOST_FRAGMENTO_APLICACION_PRODUCCION_AUDIOVISUAL_MUSICA.' .
							?instanciaST :aplicacionProduccionAudiovisualMusica ?instanciaAplicacion .
				...
				if ($errors = $GLOBALS['ONTOLOGIA_STORE']->getErrors())
					throw new Exception("Error al verificar si está siendo utilizada la instancia de la aplicación producción audiovisual y musica." .
							" Detalles:\n" . join("\n", $errors));
		...
		{
			$preMsg = 'Error al guardar la instancia de aplicación producción audiovisual y musica.';
